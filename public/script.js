...
let ts_import =
	`import {  createSystem, ts, createDefaultMapFromCDN, createVirtualTypeScriptEnvironment } from "./tsserver/tsserver.js";`;
...
export function trigger_save() {
	...
	if (session_name()) {
		localStorage.setItem(session_name(), JSON.stringify(model));
...
function group_widget(element, i, control) {
	...
	let cursor = sig(-1);
	let cursor_next = () =>
		miniStore.blocks.length > cursor() + 1
			? cursor.set(cursor() + 1)
			: cursor.set(0);
	let cursor_prev = () =>
		cursor() > 0
			? cursor.set(cursor() - 1)
			: cursor.set(miniStore.blocks.length - 1);
	...
	let remove_block = (index) =>
		setMiniStore("blocks", (e) => e.filter((r, i) => i != index));
	...
	let show_which = mem(() =>
		fold() ? null : each(() => miniStore.blocks, (e, i) => child_widget(e, i))
	);
...
function pipe_model(signal, key) {
	eff_on(
		signal,
		() => m() ? m()["function" == typeof key ? key() : key] = signal() : null,
	);
}
